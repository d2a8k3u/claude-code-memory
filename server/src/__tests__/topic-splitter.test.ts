...
describe('insertSplitSections', () => {
  ...
  it('merges with existing similar memory', async (t) => {
    if (!(await isEmbeddingsAvailable())) {
      t.skip('embedding model unavailable — cannot deterministically reproduce a merge');
      return;
    }
    // Pre-insert a memory with an embedding generated by the SAME model that
    // insertSplitSections uses, so the dedup distance is deterministically 0.
    ...
    const existingEmb = await generateEmbedding('Part A\n\nContent about part A');
    assert.ok(existingEmb, 'precondition: embedding model must produce a vector');
    db.updateMemoryEmbedding('existing', existingEmb);
    ...
    assert.equal(result.mergedCount, 1, 'Part A must merge into existing record');
    assert.deepEqual(result.mergedIds, ['existing'], 'merge must target the pre-inserted record');
    assert.equal(result.newIds.length, 1, 'only Part B must be inserted as a new record');
    assert.equal(db.countMemories(), 2, 'total = existing (merged) + Part B');
